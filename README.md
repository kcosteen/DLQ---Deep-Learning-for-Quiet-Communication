...
### Download the dataset (Kaggle API)

1. Create a Kaggle API token (`kaggle.com` → Account → *Create New API Token*) and
   put `kaggle.json` at `~/.kaggle/kaggle.json` (`chmod 600`).
2. Download and unzip into `data/raw/`:

```bash
kaggle datasets download -d grassknoted/asl-alphabet
unzip asl-alphabet.zip -d data/raw/
# result: data/raw/asl_alphabet_train/{A,B,...,Z,space,delete,nothing}/
```

Mirror if the primary is unavailable: `kaggle.com/datasets/debashishsau`.

### Build the leakage-safe split and sanity-check class balance

```bash
python -m src.data --root data/raw/asl_alphabet_train --counts \
...
python -m src.train --root data/raw/asl_alphabet_train \
...
python -m src.evaluate --checkpoint checkpoints/efficientnet_b0.pt \
    --split data/raw/asl_alphabet_train --figures docs/figures
...
```bash
python -m src.augment --image data/raw/asl_alphabet_train/A/A1.jpg --n 8
